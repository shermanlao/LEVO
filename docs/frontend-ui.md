...
| `SeriesFamilyTitle` | Series name plus **Family Datasheet**, **Installation**, and **Inquire** on the title row (`catalog.family_datasheet.download`, `catalog.installation.download`, `catalog.series.inquire`). Inquire opens `/contact?series={slug}`. |
| `SeriesConfigurator` | Series page layout: gallery left (main photo at its original ratio, max width 400px, top-aligned with thumbs and series name), compact name / Family Datasheet / Installation / description / variant selectors right (two-column grid; each field is label left + dropdown right, physical → electrical → optic → control order, no instruction copy), then `ProductList` full width. Below `lg`, selectors collapse behind a funnel on the breadcrumb row (`catalog.series.filter_toggle`). A dropdown is shown only when that spec has two or more unique values (equivalent spellings such as `120` / `120°` count as one); a complete selection shows custom datasheet / LDT URLs. The title-row Family Datasheet opens `/api/series/:slug/family-datasheet`. Installation opens `/api/series/:slug/installation` (same PDF for every SKU). LDT and the datasheet polar follow the selected beam. |
