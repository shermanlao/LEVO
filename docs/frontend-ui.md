...
The site header (`Header.tsx`) is fixed on every page, with a matching spacer so content does not sit under the bar. At the top it uses the full lockup (`py-4`, wordmark plus slogan). After you leave the top it shortens (`py-1.5`, wordmark only at `h-6`). Scrolling down hides it; scrolling up brings the short bar back. Hover, open search, the mobile menu, or keyboard focus inside the header keeps it visible. Sticky catalog filters use `--site-header-offset` so they sit below the current bar.
...
| `ProductList` | Public series combination list (cartesian of series variants). Progressive: first **25** rows, then **+50** when the visitor scrolls near the bottom (loading spinner + “Showing X of N”). Configure filter changes reset to 25. Still subject to the shared **200**-combo cartesian cap. From `lg` up: full-width `table-fixed` layout (image, wrapping SKU, then physical columns such as size / finish / trim / reflector / IP, then electrical / optic / control such as wattage / CCT / beam / dimming, then Datasheet / LDT). The SKU string uses that same segment order. SKU and size stay narrow so extra variant columns fit the browser width. CCT / beam / finish use `SpecValueIcons`. SKU text uses the same regular-weight grey as other spec cells. Below `lg`: stacked rows with the same spec values in the same physical-then-electrical-optic-control order. SKU and thumbnail open `ProductSkuDialog` for that combination. The series name is not repeated in the list (it is already in the page title). |
...
| `ImageCarousel` | Product / series gallery. The main (non-compact) photo keeps the file’s intrinsic aspect ratio (`object-contain` / `object-top`, max width 400px). Compact mode in `ProductSkuDialog` stays square. Zoom overlay close sits in the viewport safe area (not above the image) so it stays tappable on phones. |
| `SeriesFamilyTitle` | Series name. Optional `showActions`; CTAs usually render via `SeriesFamilyActions` under the description. **Family Datasheet** / **Installation** / **Inquire** (`catalog.family_datasheet.download`, `catalog.installation.download`, `catalog.series.inquire`). Inquire opens `/contact?series={slug}`. |
| `SeriesConfigurator` | Series page hero: gallery left; middle column is title → description → document CTAs; Configure aside from `lg` up (label + dropdown stay inside the panel). Below `lg`, Configure stays behind the breadcrumb funnel. Then `ProductList` full width with progressive rows (25, then +50 on scroll; heavy previews mapped only for the visible slice; filter changes reset the window). Selectors follow physical → electrical → optic → control; only kinds with two or more values are shown. Complete selection reveals custom Datasheet / LDT. |
...
| `Header` | `components/layout/Header.tsx` — sticky site bar on every page. Full height at the top; shorter when scrolled. Hides on scroll down, shows on scroll up (`use-site-header-scroll.ts`). |
| `BrandLogoMark` | `components/layout/Logo.tsx` — header/footer wordmark painted with `currentColor`. `Logo` accepts `compact` to drop the slogan and use a shorter mark. |
| `Footer` | Same-row columns: Contact Us (email / phone / address / website only when filled), Quick Links (Products, Projects, About, Contact Us), **Resources** (Warranty / Certifications / Technical Underneath, stacked like Quick Links), and **Media** icons when URLs are set (LinkedIn, Facebook, Instagram, Threads, Pinterest; hide empty). Then logo/copyright. Resource and social links use `HelpLink`. |
| `UnderConstruction` | Public-page body while `public_under_construction` is on and the visitor is not signed in. Header and footer stay. Centered heading with cone, pendant-light, and hard-hat stroke icons in square black frames (same outline language as `.btn-secondary`). |
...
| `ImageFileIntake` | Admin image placeholder intake: drop a photo, paste from the clipboard while hovered/focused, or click an empty placeholder to choose a file. Filled slots keep click-to-enlarge. |
...
| `SeriesFeaturedImageEditor` | Series admin: one source upload, then 16:9 / 4:5 / 1:1 crops. Each empty slot is a drop / paste / choose placeholder. **Replace photo** appears after a crop is saved. Used on Add / Edit Series, not on list-row thumbs. |
| `AdminPhotoSlot` | Admin photo cell. Default frame is square (`aspect-square`). Pass `frameClassName` for other ratios (catalog 16:9, series page 4:5). Hover shows a larger preview; click opens `ImageLightbox` (hover preview hides while the lightbox is open). Empty slots show **Drop, paste, or choose a file**. Used by size-pack, appearance photos, and series featured thumbs. `AdminHoverPreview` wraps other admin images for the same hover enlarge. |
| `SizePackPhotos` | Series size rows: square Main A/B and Size drawing. Drop, paste, or click an empty placeholder to upload (no separate Upload button). Size drawing **Generate by AI** sends Main A, dimensions, cutout, series Description, and the filled Phrase template. Filled Main A/B show **Match catalog style** (needs a catalog style photo on `/admin/ai`). |
| `StyleReferenceUploader` | Shared `/admin/ai` style photo slot (size drawing and catalog photo). Crop at 1:1, then POST/DELETE the style endpoint. Catalog photo style has its own prompt textarea under the card (`admin.ai.product_photo_style_prompt`). |
| `AppearancePhotos` | Series Finish × Trim × Reflector photos, directly under Size. Square thumbs (drop / paste / click empty placeholder), Generate by AI (pending preview, Confirm to save), Generate missing, Generate all, Confirm all / Discard all, Remove. Unused leftover photos from cleared tags are listed separately. Hidden when those kinds are N/A and no unused photos remain. |
