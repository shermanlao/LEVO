...
| `SeriesFeaturedImageEditor` | Series admin: one source upload, then 16:9 / 4:5 / 1:1 crops. Each slot can upload or replace its own photo, or adjust the crop from the source. `layout="thumb"` is the list-row square placeholder (same drop / paste / choose slot as size Main A/B). |
