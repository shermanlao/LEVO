...
| `SeriesFamilyTitle` | Series name on its own line, then compact **Family Datasheet** / **Installation** / **Inquire** in a wrap row (`catalog.family_datasheet.download`, `catalog.installation.download`, `catalog.series.inquire`). Inquire opens `/contact?series={slug}`. |
| `SeriesConfigurator` | Series page hero: gallery left (intrinsic-ratio photo, capped width), identity column (title, document CTAs, description), and a compact Configure aside on the right from `lg` up (label left + equal-width dropdowns, single column). Below `lg`, Configure stays behind the breadcrumb funnel. Then `ProductList` full width. Selectors follow physical → electrical → optic → control; only kinds with two or more values are shown. Complete selection reveals custom Datasheet / LDT. Family Datasheet and Installation stay on the title actions. |
