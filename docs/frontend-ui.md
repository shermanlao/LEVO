...
| `SeriesFamilyTitle` | Series name. Optional `showActions`; CTAs usually render via `SeriesFamilyActions` under the gallery. **Family Datasheet** / **Installation** / **Inquire** (`catalog.family_datasheet.download`, `catalog.installation.download`, `catalog.series.inquire`). Inquire opens `/contact?series={slug}`. |
| `SeriesConfigurator` | Series page hero: gallery left with document CTAs under the image; identity column (title then description); Configure aside from `lg` up (label + dropdown stay inside the panel). Below `lg`, Configure stays behind the breadcrumb funnel. Then `ProductList` full width. Selectors follow physical → electrical → optic → control; only kinds with two or more values are shown. Complete selection reveals custom Datasheet / LDT. |
