...
| `AdminPhotoSlot` | Square admin photo cell (`aspect-square`). Hover shows a larger preview; click opens `ImageLightbox` (hover preview hides while the lightbox is open). Used by size-pack and appearance photos. `AdminHoverPreview` wraps other admin images for the same hover enlarge. |
