...
4. **Shortcuts** — Catalog (Product Types, Series, **Variant**, plus partner catalog and LDT library), Projects (plus Contact inquiries), Settings, **AI** (**Open AI settings** → `/admin/ai`), Users (admin role only). Settings opens `/admin/settings` for brand, homepage, contact, and SEO. See [admin-site-settings.md](admin-site-settings.md). **Variant** (`/admin/variant-options`) is the global option+code catalog; series pages add those options as tags. See [variant-options.md](variant-options.md).
