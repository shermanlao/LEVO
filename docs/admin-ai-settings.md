...
- Optional catalog **photo style** (`product_photo_style_image`) plus an editable prompt (`product_photo_style_prompt`). Upload on `/admin/ai` stores `/images/ai/product-photo-style.{png|jpg|webp|gif}`. The placeholder accepts drop, clipboard paste, or a chosen file. The prompt sits under that card (same Reset-to-default pattern as size drawing). Empty stored values, or a previous built-in default, use the 3-step prompt (cutout hidden parts → polish the cutout → place into the reference scene). Placeholders: `{{hints_line}}`, `{{placeholder_size}}`. A lock is always prepended so `<IMAGE_0>` stays the original fixture and `<IMAGE_1>` stays look/scene only. Match catalog style first asks a chat/vision model to describe the original photo (uses Description phrase failover, not the Imagine image model), then sends that text plus the filled series **Phrase template** (this size pack plus other tags) and the square **1:1 catalog placeholder** size (1600×1600, same as Main A / Main B). After the styled preview appears, staff can Refine with chat via `edit-product-photo`. Match catalog style on Main A / Main B is optional and never required to save an upload. Style match shrinks both photos to JPEG (max edge 1600) before the provider call. xAI gets the product as `<IMAGE_0>` and the style reference as `<IMAGE_1>` (`sourceFirst`). Failures show the provider text instead of a generic server error.
