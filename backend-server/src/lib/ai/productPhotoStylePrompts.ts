...
export function isLegacyProductPhotoStylePrompt(template: string): boolean {
  ...
  return Boolean(stored) && stored === normalizePromptText(LEGACY_PRODUCT_PHOTO_STYLE_PROMPT);
}
...
export function fillProductPhotoStylePrompt(
  ...
  const vars = productPhotoStylePromptVars(hints, phrase, photoDescription);
  const body = fillPromptTemplate(template, {
    ...vars,
    phrase_line: '',
    photo_description_line: '',
  });
  return [PRODUCT_PHOTO_STYLE_LOCK, vars.phrase_line, vars.photo_description_line, body]
    .filter(Boolean)
    .join('\n');
}
